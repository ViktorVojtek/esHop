...
const ProductImages: FC<IProductImages> = ({
  ...
  const [images, setImages] = useState(productData ? productData.images : []);

  useEffect(() => {
    setImages(productData ? productData.images : []);
  }, [productData]);
  ...
  const handleImageData = async (event) => {
    ...
    const bytesToSize: (bytes: number) => string = (bytes) => {
  ...
  const handleRemoveImage: (idx: number) => Promise<void> = async (idx) => {
    const removeItem: (items: ProductImage[], i: number) => ProductImage[] = (
      items,
      i
